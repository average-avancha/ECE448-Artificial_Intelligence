...
def computeEmission(tag, word, emission, smoothing, tag_count, word_count, hapax, hapax_n):
    """
    ...
    n = tag_count[tag]
    V = len(word_count)
    alpha = smoothing * computeHapax(tag, hapax, hapax_n, smoothing)
...
def buildTrellis(sentence, all_tags, all_words, min_prob, default_tag, tuning_constant, initial_word_count, hapax_n, initial, transition, emission, hapax):
    """
    Hidden Markov Model:
        P(T∣W) ∝ P(W∣T) ∗ P(T) 
        = ∏i=[1, n] {P(wi∣ti) ∗ P(t1∣START) ∗ ∏k=[2, n] {P(tk∣tk−1)}}
        ...
        if i == 0:
            ...
            for prev_tag in all_tags:
                for tag in all_tags:
                    ...
                    emission_p = computeEmission(tag, word, emission, tuning_constant, all_tags, all_words, hapax, hapax_n)
